...
BEGIN
  ...
  UPDATE players SET tribe = 'Cila', tribe_color = '#FF6B35'
    WHERE season_id = s50 AND name IN (
      'Rizo Velovic', 'Dee Valladares', 'Cirie Fields',
      'Kamilla Karthigesu', 'Rick Devens', 'Charlie Davis',
      'Jonathan Young'
    );
  ...
  UPDATE players SET tribe = 'Kalo', tribe_color = '#4A90D9'
    WHERE season_id = s50 AND name IN (
      'Aubry Bracco', 'Colby Donaldson', 'Benjamin "Coach" Wade',
      'Chrissy Hofbeck', 'Tiffany Ervin', 'Genevieve Mushaluk',
      'Joe Hunter'
    );
